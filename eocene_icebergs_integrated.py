'''
Code for Eocene Antarctic iceberg simulations
---------------------------------------------
#(!)#  marks changes required every run (iceberg size, filename)
#(-)#  marks changes required for forward sensitivity runs (shifted location, adapted basal melt)
#(?)#  marks changes required to run backward simulations
#(?-)# marks changes required for backward sensitivity runs
'''
...
pclass = particleclass.Iceberg_FW # or: Iceberg_BW
...
#                  -57.04998779, -57.0249939,  -57.,         -56.9750061,  -56.95001221,
#                  -57.04998779, -57.0249939,  -57.,         -56.9750061,  -56.95001221,
#                  -57.04998779, -57.0249939,  -57.,         -56.9750061,  -56.95001221])# -1. # Sensitivity study - shifted location #(?-)#
...
#                  -67.48478699, -67.48478699, -67.48478699, -67.48478699, -67.48478699])# -0.5 # Sensitivity study - shifted location #(?-)#
...
# fieldset.add_constant('ODP_lat', -67.)        # Shifted latitude ODP 696 Eocene [°] # Sensitivity study - shifted location #(-)#
# fieldset.add_constant('ODP_lon', -58.)        # Shifted longitude ODP 696 Eocene [°]
...
## Sensitivity study - adapted basal melt #(-)#
# fieldset.add_constant('Lf', 3.34e5)           # latent heat of fusion [J/kg]
# fieldset.add_constant('cpo', 4004.)           # specific heat capacity ocean [J/(kg K)]
# fieldset.add_constant('cd', 1.5e-3)           # drag coefficient [-]
# fieldset.add_constant('xiN', 0.052)           # stability constant [-]
# fieldset.add_constant('Rc', 0.20)             # critical flux Richardson number [-]
# fieldset.add_constant('k_r', 2.5)             # reciprocal of the von Kármán's constant (0.40) [-]
# fieldset.add_constant('nu', 1.826e-6)         # kinetmatic viscosity polar ocean [m2/s]
# # fieldset.add_constant('koT', 0.563)           # thermal conductivity ocean [W/m]
# fieldset.add_constant('Pr', 13.3435)          # Prandtl number [-]
# fieldset.add_constant('Om', 7.92921e-5)       # 
# fieldset.add_constant('Lo', 100.)             # Obukhov length [m]
...
melt_terms = pset.Kernel(kernels.BuoyantConvection) + \
    pset.Kernel(kernels.BasalMelt) + pset.Kernel(kernels.WaveErosion) # Sensitivity study - adapted basal melt: use BasalMeltAlt #(-)#
...
#==================== EXECUTE ====================#
### Define output file
out = pset.ParticleFile(name='25p_5y_int_30d_1hdtar_C4g.zarr', outputdt=delta(days=30)) # #(!)#

### Execute #(?)#
## Forward
totalKernel = properties + pset.Kernel(kernels.SampleFields) + melt_terms + pset.Kernel(kernels.Grounding) + dynamic   # Grounding
...
## Backward
# totalKernel = properties + pset.Kernel(kernels.SampleFields) + pset.Kernel(kernels.SampleRegion) + melt_terms + dynamic
# pset.execute(totalKernel,
#              runtime=delta(days=(len(dr)-2)),
#              dt=delta(hours=-1),
#              output_file=out,
